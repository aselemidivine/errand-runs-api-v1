...
SQL Server is the system of record. Each module owns a schema: `identity`, `users`, `app` (errands), `runners`, `kyc`, `matching`, `tracking`, `payments`, `memberships`, `notifications`, `reviews`, `support`, and `admin`. The current migration model includes Errands/ErrandStops/ErrandItems, RunnerProfiles, customer Payments, RunnerLedger, RunnerPayoutAccounts and RunnerPayouts; remaining tables are later slices and are not claimed as implemented.
